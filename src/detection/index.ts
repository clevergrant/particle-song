...
import { STABILITY_ALPHA, STABILITY_EPSILON } from "../constants";
...
export interface DetectionConfig {
  ...
  /** How long an organelle's particle IDs are held after the organelle
   *  breaks apart, so brief coherence dropouts don't fragment identity. */
  readonly organelleLatchSeconds: number;
...
export function runDetection(
  ...
  const { organelles, holdTimers } = detectOrganelles(data, prevFrame, config, dt, config.organelleLatchSeconds);
 ...
 * organism's age. Downstream consumers may filter on this.
 *
